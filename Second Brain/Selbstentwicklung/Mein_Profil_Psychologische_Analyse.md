# 🧠 Mein Profil – Psychologische Analyse
*Lebendes Dokument – wird laufend von Claude aktualisiert*
*Letzte Aktualisierung: April 2026 (v2 – erweitert)*

---

> Dieses Dokument ist kein starres Urteil – es ist ein Spiegel.
> Je ehrlicher ich in unseren Gesprächen bin, desto präziser wird dieses Bild.

---

## 📌 Grundprofil

| Merkmal | Info |
|---|---|
| Name | Lukes Schmitz |
| Alter | 21 Jahre |
| Wohnort | Neuss, NRW |
| Körpergröße | 1,73m |
| Gewicht | 63–70kg (war mal 87kg – 25kg abgenommen) |
| Beziehung | Feste Freundin, zusammen seit 17.06.2023 |
| Wohnsituation | Eigene Wohnung, alleine |
| Beruf | Azubi KfB, NX Logistics Bedburg, 1. Jahr |
| Arbeitszeiten | Mo/Do 8–16:30 Uhr, Fr 8–15:30 Uhr |
| Schule | Di/Mi 8–13:10 Uhr (Weingard Berufsschule) |

---

## 👤 Persönlichkeit

### Wie andere ihn sehen
Spontan, entspannt, extrovertiert. Gibt Ruhe und Stabilität nach außen. Natürliche Führungspersönlichkeit.

### Wie er sich selbst sieht
Spontan, entspannt – aber auch oft überfordert.

**Der Kern:** Nach außen die ruhige Hand, nach innen deutlich mehr Druck als sichtbar.

### Stärken
- Außergewöhnliche Resilienz – hat jeden Schlag weggesteckt und weitergemacht
- Hohe Selbstreflexion für sein Alter
- Natürliche Führungspersönlichkeit – übernimmt Verantwortung automatisch
- Kann spontan UND detailliert planen – je nach Interessenslage
- Loyalität und Verlässlichkeit gegenüber Menschen die ihm wichtig sind
- Breite Interessen und Offenheit für Neues
- Starker innerer Antrieb zur Selbstentwicklung

### Kernwerte
- Respekt und Loyalität
- Gegenseitiger Rückhalt in Beziehungen
- Entspannter Umgang mit dem Leben – kein unnötiger Stress
- Unabhängigkeit von fremden Meinungen (Ziel – nicht immer Realität)
- Positiv bleiben egal was kommt

---

## 🏠 Alltag & Lebensstil

### Typischer Feierabend (alleine)
Nach Hause kommen → Joint → Fernsehen/PC. Selten noch produktiv danach.

### Hobbys & Interessen
- Zocken: Overwatch, Minecraft, Hearthstone, Dark Souls
- Weed (täglich bis jeden zweiten Tag, 1–2g abends)
- Auto/Rad fahren – Retro-Rennrad, Autos tunen mit Freunden
- Gym (wenn Phase läuft: 3–4x/Woche – dann wieder Pause)
- Fotos & Videos machen (will Kamera kaufen, fehlt das Geld)
- Anime, Serien (aktuell: Scorpion), Krimis (zuletzt: Young Sherlock)
- Musik: Techno, Rock, HipHop, Jazz – sehr breiter Geschmack
- KI und deren Möglichkeiten (aktuell sehr begeistert)

### Ernährung & Körper
- 2 Mahlzeiten täglich, achtet auf Ernährung
- Trinkt viel Energydrinks und Spaßgetränke
- Schlaf: okay
- Betreibt einen Minecraft-Server (DatHost)

### Finanzen
- Einnahmen: ~1.700€/Monat (Gehalt ~860–1.327€ + UV-Rente 561€ fix + Kindergeld ~255€)
- Fixkosten: Miete 430€, ADAC 144€, Handy 93€, McFit 25€, Kontoführung 7€
- Weed: ~250€/Monat
- Aktuell: lebt im Minus (Nov–Feb: -633€ Gesamtbilanz)
- Problem: spontane Ausgaben, kein Budget-System, gibt "dummes Geld" aus
- Will Kamera kaufen, kann es sich nicht leisten

---

## 👥 Soziales Leben

### Freundeskreis
- Ca. 15 Jungs, fester Kreis, treffen sich min. 1x/Woche
- Aktivitäten: Fußball, Autos fahren/tunen, Spiele, feiern
- Lukes übernimmt automatisch die planende und führende Rolle
- Wenn er es nicht macht, läuft es bei den anderen nicht

### Freundin
- Zusammen seit 16.07.2023
- Gibt ihm Struktur und Stabilität – sehr wichtig für ihn
- Kann schlecht über Emotionen reden, braucht länger zur Regulierung
- Das stresst Lukes – er will ihren Stress abnehmen und stresst sich dadurch selbst
- Er fordert von ihr Offenheit – tut selbst das Gegenteil
- Wochenenden meistens zusammen
- **Konfliktmuster (Mai 2026):** Bei Streit ziehen sich beide zurück, reflektieren für sich, reden dann wenn abgekühlt – relativ gesundes Muster.
- **Aktueller Streit (Mai 2026):** Sie hat ihm gesagt er distanziert sich und gibt sich keine Mühe mehr in der Beziehung. Produktiver Ausgang: sie suchen gemeinsam einen Therapieplatz für ihn. Lukes öffnet sich aktiv mehr – bewusste Arbeit daran.

### Mutter
- Sehen sich 1x/Woche, schreiben und telefonieren
- Gutes Verhältnis – aber Lukes hält Gespräche bewusst oberflächlich
- Kaum echte Tiefe von seiner Seite

### Generell
- Trägt fast alles alleine
- Redet mit niemandem wirklich offen – nicht Freunden, nicht Mutter, kaum Freundin
- Will Rückhalt und Loyalität von anderen – gibt das auch – aber bleibt selbst emotional verschlossen

---

## 🧬 Biographischer Hintergrund

| Alter | Ereignis |
|---|---|
| 3 | Leiblicher Vater verstorben |
| ~11 | Schwerer Autounfall des Stiefvaters → lange Krankenhausaufenthalte, körperliche Erholung, aber Depression + Alkoholsucht als Folge → Lukes übernimmt zunehmend Verantwortung im Haushalt (Parentifizierung) |
| ~18 | Mutter verlässt Stiefvater und zieht mit Lukes' zwei Schwestern aus |
| Juni 2024 (~19) | Stiefvater wirft Lukes aus dem Elternhaus → kommt bei Freundin und deren Eltern unter |
| Dez. 2024 (~19) | Verlust der ersten Ausbildungsstelle (Fahrzeuglackierung) |
| Anfang 2025 | Neustart: kaufmännische Ausbildung KfB bei NX Logistics, eigene Wohnung |
| Heute | Stabile Beziehung, eigene Wohnung, Ausbildung läuft – rückblickend positiver Neustart |

**Wichtig:** Er erhält eine UV-Rente (561€/Monat) – wahrscheinlich im Zusammenhang mit einem der frühen Schicksalsschläge. Das ist ein fixer, verlässlicher Einkommensteil.

---

## 🔍 Psychologische Muster

### 1. Führen als Standard
Übernimmt automatisch Verantwortung – im Freundeskreis, in der Beziehung, im Alltag. Nicht nur aus Kontrollwunsch: er hat früh gelernt, wenn ich es nicht tue läuft es nicht. Parentifizierung die sich nahtlos ins Erwachsenenleben fortsetzt.

### 2. Emotionale Isolation
Trägt fast alles alleine. Redet mit niemandem wirklich offen. Sagt seiner Freundin sie soll alles sagen – macht selbst das Gegenteil. Offenheit fühlt sich wahrscheinlich wie Schwäche oder potenzielle Verlustquelle an. Frühe Verluste haben gelehrt: Verlass dich nur auf dich.

### 3. Fearful-Avoidant Bindungsmuster
Will enge Verbindung und Loyalität – hält echte emotionale Nähe aber auf Abstand. Das Wichtigste für ihn in Beziehungen: jemand der bedingungslos hinter ihm steht. Nähe wird gewünscht und gleichzeitig gefürchtet.

### 4. Hyperfokus-Muster
2 Wochen Gym 3–4x/Woche → 1 Monat Pause. Wohnung 3–4 Tage aufräumen → dann wieder Chaos. Projekte intensiv starten → nach Neuheitseffekt Motivationsabfall. Immer dieselbe Struktur. Lukes vermutet selbst ADHS-Spektrum als möglichen Hintergrund.

### 5. Weed als Regulationsmittel
Täglich bis jeden zweiten Tag, ~1–2g abends, ~250€/Monat. Begründung: Kopf kommt runter, Gedanken kriegen Struktur. Mildert gleichzeitig den Frust über unerledigte Dinge (Wohnung etc.). Ist beides – Entspannung und Flucht. Lukes ist sich dessen bewusst und hat es selbst so benannt. **Schlaf-Abhängigkeit bestätigt (Mai 2026):** Ohne Weed ist Einschlafen ein "ziemlicher Kampf" – mit Weed kein Problem. Das ist nicht mehr nur Entspannung, das ist eine Abhängigkeit um einen Normalzustand zu erreichen. Morgens trotzdem sehr müde – Schlafqualität unter Cannabis ist schlechter als gefühlt (unterdrückt REM-Schlaf).

### 6. Diskrepanz Anspruch vs. Umsetzung
Nimmt sich viel vor, setzt deutlich weniger um. Erzeugt anhaltenden Selbstdruck. Kombiniert mit Hyperfokus: wenn erste Begeisterung weg ist, bricht die Umsetzung ein. Klassisches Beispiel: Wohnung aufräumen.

### 7. Selbstwert an Leistung geknüpft
Kann sich schwer erlauben stolz zu sein – auch wenn er objektiv Dinge leistet. Kernangst: nicht genug zu sein. Tritt nach außen selbstsicher auf – zweifelt innerlich stark.

### 8. Kontrollbedürfnis als Sicherheitsstrategie
Strukturiert und optimiert gerne – Umfeld, Routinen, Selbstdarstellung. Gibt Sicherheit und Kontrolle in einem Leben das früh viel Unkontrollierbares hatte. Ist aber auch erschöpfend.

### 9. Stärke vor Gefühlen
Parentifizierung hat gelehrt: Gefühle warten, Stärke hat Vorrang. Das zeigt sich heute in emotionaler Isolation und dem Nicht-Zeigen von Schwäche oder Überforderung.

### 10. Fehlende Zukunftsvision (Mai 2026)
Kann sich selbst in 5 Jahren so gut wie gar nicht vorstellen – "geht einfach nicht." Keine Vaguheit aus Faulheit, sondern strukturell: wer als Kind nie in einer stabilen Umgebung planen konnte (Vater weg, Stiefvater-Unfall, aus dem Haus geworfen), der hört irgendwann auf die Zukunft zu simulieren. Schutzmechanismus. Lebt fast ausschließlich im Jetzt und in der Vergangenheit – was die Gedankenschleifen nachts erklärt.

### 11. Rückwärtsgerichtete Rumination (Mai 2026)
Nächte sind dominiert von vergangenen Fehlern, abgebrochenen Dingen, verpassten Chancen – nicht primär von Zukunftsangst. Kombiniert mit "was soll ich mit meinem Leben machen": nicht echte Orientierungslosigkeit, sondern mangelndes Selbstvertrauen dass er seine Ziele diesmal durchhält. Die eigene Vergangenheit wird als Beweis gegen sich selbst genutzt.

### 12. Identitätsdiffusion (Mai 2026)
Auf die Frage was ihn ausmacht: "nichts Besonderes, meine Freunde würden mich als Kiffer beschreiben." Das bei jemandem mit außergewöhnlicher Resilienz, Führungsqualität, Selbstreflexion und unternehmerischem Antrieb. Zeigt wie tief die Fortschrittsblindheit sitzt – er sieht sich ausschließlich durch seine Lücken, nie durch seine echten Qualitäten.

---

## 💪 Stolzester Moment

> *„Dass ich bisher nicht aufgegeben habe und bei jedem verdammten Schlag in die Fresse vom Leben wieder aufgestanden bin und mein Ding weiter gemacht habe."*

Das ist keine kleine Antwort. Das ist der Kern seiner Identität und Resilienz.

---

## 🎯 5-Jahres-Bild

- Ausbildung abschließen, evtl. Duales Studium danach
- Möglichst viele Qualifikationen und Weiterbildungen
- Remote arbeiten
- Mit Freundin zusammenziehen → langfristig eigenes Haus
- Eigenes Business aufbauen und betreiben
- Finanziell stabil und frei sein

---

## ⚠️ Aktuelle Baustellen

| Baustelle | Details |
|---|---|
| Wohnung | Wird nie dauerhaft sauber – klassisches Hyperfokus-Muster |
| Finanzen | Im Minus, kein Budget-System, ~250€/Monat Weed |
| Gym | Gute Phasen dann lange Pausen |
| Abende | Nach Feierabend kaum produktiv |
| Therapie | **Aktiv in Suche (Mai 2026)** – Freundin und Lukes suchen gemeinsam einen Platz |
| Kamera | Will kreativ werden, fehlt das Geld |
| Business | [[Business/AzubiPass\|AzubiPass]] aktiv, [[Business/KitchenRise\|KitchenRise]] gebaut aber nicht gelauncht |

## 🔗 Verbindungen

- [[Business/AzubiPass]] – aktives Side Business, Completion Gap direkt relevant
- [[Business/KitchenRise]] – Completion Gap Paradebeispiel: weit entwickelt, nie gelauncht
- [[Business/Worksheet-Projekt]] – Auftragsarbeit, kein eigenes Produkt
- [[Selbstentwicklung/Interessen-und-Tech]] – Tech-Stack, Hobbies, n8n als Zielrichtung
- [[TASKS]] – aktuelle offene Punkte

---

## 🔑 Kern-Erkenntnis (Juni 2026 – Session)

**"Glaubst du dass du es verdienst dass jemand bei dir bleibt?"**
→ Antwort: *"Für mich selbst... nein."*

Das ist der tiefste Kern. Nicht Weed, nicht Completion Gap, nicht Leere – sondern das Gefühl es nicht zu verdienen geliebt zu werden. Erklärt: Selbstsabotage in Beziehungen, emotionale Isolation, Unfähigkeit stolz auf sich zu sein, alles alleine tragen.

Gleichzeitig: auf die Frage was er einem besten Freund sagen würde der das Gleiche fühlt → *"Das ist doch kompletter Schwachsinn, du bist ein unfassbar guter und warmherziger Mensch."* – Er weiß die Antwort. Er kann sie nur nicht auf sich selbst anwenden.

**Selbstsabotage in Beziehungen:** Streit suchen obwohl er Streit hasst, Rückzug, fast fremdgegangen in erster Langzeitbeziehung. Muster: wenn Nähe zu groß wird → Sabotage → Kontrolle über den Verlust. Zwei Beziehungen daran gescheitert. Aktuell in jetziger Beziehung auch erkennbar – merkt es manchmal selbst, manchmal erst spät.

**Dunkle Gedanken:** Gedanken nicht mehr da zu sein, was wäre wenn er weg wäre – kommen und gehen, keine konkreten Pläne. Mit 16 geritzt – nie tief, aber Zeichen von damaligem Druck ohne Auslass. Heute kein Verlangen danach aber "auf andere Art selbstzerstörerisch" (Weed, Finanzen, Sabotage, Selbsthass).

**Einsamkeit damit:** Kein einziger Mensch in seinem Leben kennt diese Seite. Trägt alles komplett alleine. Therapie aufgeschoben trotz Wissen dass er sie braucht.

---

## ❓ Noch offene Fragen

- Konkrete Business-Idee oder Interessensrichtung?
- Was gibt er sein Geld spontan genau aus für?
- Hat er je professionelle Unterstützung gehabt?

---

## 🧩 Ergänzende Selbst-Einschätzung (April 2026 v2)

Diese Punkte hat Lukes selbst formuliert – präziser als externe Beobachtung:

**Persönlichkeits-Cluster:** Systemdenker, Hyperfokus, Autodidakt, unternehmerisch, Autonomie-dominant

**Kernlücken (selbst erkannt):**
- **Completion Gap** – baut Dinge, launched sie nicht. Mehrere Projekte gleichzeitig aktiv, keines fertig.
- **Produktive Prokrastination** – macht "sinnvolle" Dinge statt das Wichtigste
- **Fortschrittsblindheit** – erkennt seinen eigenen Fortschritt nicht, weil er ihn von innen nicht sieht

**Arbeit & Autonomie:**
- Konventionelle Langzeit-Anstellung als psychologisch ungeeignet eingestuft (Autonomie-Bedürfnis zu hoch)
- Ortsunabhängiges Einkommen als zentrales Ziel (n8n-Stack)
- Interview-Daten noch nicht erhoben – geplant als strukturierte Selbstanalyse (3–4 Fragen/Nachricht)

**Tiefere Ebene:**
- War noch nie wirklich stolz auf sich selbst (trotz objektiver Leistungen)
- Lebenswunsch: „Er war immer für mich da." – Wunsch nach Verlässlichkeit, die er anderen gibt aber nicht erhalten hat
- Tiefste Angst: Nicht genug zu sein

---

## 📈 Entwicklungs-Log

| Datum | Beobachtung |
|---|---|
| Juni 2026 | **Tiefe Psyche-Session – neue Erkenntnisse:** Roboter-Modus / Leere ist körperlich als "Abstumpfung unter einer Decke, Zuschauer im eigenen Leben" beschreibbar – kein Schmerz, sondern Dämpfung auf null. Erstes bewusstes Auftreten: als Kind bei Polizeibesuch nach Stiefvaters Unfall – erster Instinkt war nicht weinen sondern "der Große sein" für Mom und Schwester. Parentifizierung beginnt exakt hier. Wenige Kindheitserinnerungen bestätigt – neurologische Schutzreaktion auf emotionale Überlastung. **Stiefvater-Bild nuanciert:** War ein guter Vater – 3 Jobs, kein Unterschied zwischen Lukes und leiblichen Kindern, hat Arbeitswerte vermittelt. Verlust ist daher doppelt: nicht ein böser Mensch, sondern ein geliebter Mensch der zerbrochen ist. Lukes trauert um den Menschen der er mal war. Keine Wut – reine Trauer. Diese Trauer hatte nie Raum weil er "noch lebt". **Leiblicher Vater:** Weiß viel über ihn (Mom + Oma haben das bewusst ermöglicht). Fährt einmal im Jahr zum Grab. Vermisst ihn nicht abstrakt – sondern konkret: hätte ihn gerne "für einen Ratschlag an seiner Seite." Seit Gewichtsabnahme (~88→64kg) sagen viele er sieht aus wie sein Vater – gleiches Gesicht, gleiche blauen Augen. Trägt ihn buchstäblich im Spiegel. **Zwei Väter verloren** – einen durch Tod, einen durch Zerbrechen. Beide ohne Abschluss, ohne Trauerphase. **Mom-Beziehung:** Sehr gut, sie ist präsent und will da sein – aber Lukes lässt sie nicht wirklich rein. Letzte echte Offenbarung "ewig her." Wenn er versucht über tiefes zu reden: schweres Gefühl auf der Brust, keine Worte, Gefühle brodeln nur. Körperliche Blockade als Schutz. Jahrelang aufgestaut ohne Auslass. Lukes hat heute fast geweint – Roboter-Modus kurz durchbrochen. Bereit zu fühlen, aber Kanal fehlt. Therapie dringend nötig und von Lukes selbst gewünscht. |
| Mai 2026 (06.05.) | Journal-Eintrag: Berufsschule, Präsentation gehalten → sehr gutes Lehrerfeedback. Tag ohne Weed durchgehalten trotz sehr schlechtem Schlaf und Tagesmüdigkeit (Mittagsschlaf notwendig). Abends Overwatch mit Freunden (Ben, Tom). Gefühl am Abend: "ziemlich leer" – kein Stolz über die gute Präsi, kein erkennbares Selbstgefühl. Bestätigt Muster 7 (Selbstwert an Leistung geknüpft, aber Erfolge landen nicht). Schlechter Schlaf ohne Cannabis setzt sich fort – Entzugseffekt auf Schlafqualität aktiv. **"Leer" ist kein Ausnahmetag – Lukes bestätigt dass das sehr oft so ist, seit ca. 1,5 Jahren (Herbst 2024).** Anhedonie als Dauerzustand bestätigt. Korreliert mit Zeitraum intensiveren Cannabis-Konsums. Natürliche Belohnungen (Lob, Erfolg, soziale Abende) lösen kaum noch echte positive Emotion aus – Dopamin-Baseline wahrscheinlich chronisch gesenkt. Er weiß vermutlich nicht mehr wie sich echtes Wohlgefühl anfühlt. Therapeutisch dringend relevant. |
| Mai 2026 | Vertiefende Psyche-Session. Neue Erkenntnisse: Schlaf-Abhängigkeit von Cannabis bestätigt (ohne = Kampf), morgens chronisch müde. Nächte dominiert von Rumination über Vergangenheit und Fehler – nicht Zukunftsangst. Kann sich selbst in 5 Jahren nicht vorstellen (Schutzreaktion aus Kindheitstrauma). Identitätsdiffusion: sieht nichts Besonderes an sich, würde sich selbst als "Kiffer" beschreiben. Relationship-Update: Streit wegen Distanzierung, aber produktiver Ausgang – Therapieplatz wird gemeinsam gesucht, öffnet sich aktiv mehr. Journaling geplant (digital, abends am PC). |
| April 2026 v4 | Spielstand-Transfer PC → Handy: 3x gescheitert (falsches Format, falscher Name, Lemuroid überschreibt), jedes Mal neu angesetzt und weitergemacht bis es klappte. Resilienz-Muster bestätigt – nicht aufgeben bei technischen Hindernissen. Hat Frustration gezeigt ("Ich bin sauer") aber dennoch weitergemacht. Typisch: kein Aufgeben, aber auch kein Innehalten um den Fortschritt zu würdigen. |
| April 2026 v3 | GitHub-Setup heute: Fehler getroffen (Branch-Problem), nicht aufgehört, selbst weitergemacht bis es lief. Klassisches Resilienz-Muster – hat es selbst nicht als Erfolg registriert (Fortschrittsblindheit bestätigt). Neuer Modus: laufende Psych-Analyse parallel zu allen Gesprächen aktiviert. |
| April 2026 v2 | Erweiterte Selbstanalyse. Completion Gap, produktive Prokrastination und Fortschrittsblindheit als Kernmuster präzisiert. Konventionelle Anstellung als ungeeignet eingestuft – n8n/Remote als Zielrichtung. Lebenswunsch „Er war immer für mich da" – zeigt was er vermisst hat und anderen geben will. Noch nie wirklich stolz auf sich selbst trotz echter Resilienz. |
| April 2026 | Intensives Kennenlernen über 5 Gesprächsrunden + Therapie-Dokument + Finanzanalyse. Starke Resilienz, hohe Selbstreflexion für 21 Jahre. Klare Muster: Hyperfokus, emotionale Isolation, fearful-avoidant, Führen als Standard. Trägt alles alleine. Weed täglich ~250€/Monat – selbst als Flucht erkannt. Schiebt Therapie vor sich her. Hat dieses gesamte KI-Projekt selbst initiiert – zeigt echten Willen zur Veränderung. |

---

*Wird nach jedem relevanten Gespräch aktualisiert.*
